...
def transform_claims(df_bronze: DataFrame) -> DataFrame:
    ...
    return (
        df_bronze.filter(F.col("claim_id").isNotNull())
        ...
        .withColumn(
            "closed_date",
            F.when(F.col("closed_date") == "", None).otherwise(
                F.col("closed_date").cast(DateType())
            ),
        )
...
def run(
    ...
    target_table: str = "fintech_catalog.silver.cleaned_claims",
) -> int:
